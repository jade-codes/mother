...
fn test_with_git_info_valid_repo() {
    ...
    let commit_oid =
        create_test_repo_with_commit(repo_path, "main").expect("Failed to create test repo");
...
fn test_with_git_info_non_existent_directory() {
    ...
    assert!(
        scan.commit_sha.is_none(),
        "commit_sha should remain None for non-existent directory"
    );
    assert!(
        scan.branch.is_none(),
        "branch should remain None for non-existent directory"
    );
}
...
fn test_with_git_info_non_git_directory() {
    ...
    assert!(
        scan.commit_sha.is_none(),
        "commit_sha should remain None for non-git directory"
    );
    assert!(
        scan.branch.is_none(),
        "branch should remain None for non-git directory"
    );
}
...
fn test_with_git_info_subdirectory_discovers_parent_repo() {
    ...
    assert!(
        scan.commit_sha.is_some(),
        "commit_sha should be populated from parent repo"
    );
    assert_eq!(
        scan.commit_sha.unwrap(),
        commit_oid.to_string(),
    ...
    assert!(
        scan.branch.is_some(),
        "branch should be populated from parent repo"
    );
}
...
fn test_with_git_info_detached_head() {
    ...
    assert!(
        scan.commit_sha.is_some(),
        "commit_sha should be populated even in detached HEAD"
    );
    assert_eq!(
        scan.commit_sha.unwrap(),
        commit_oid.to_string(),
...
fn test_with_git_info_empty_repo() {
    ...
    assert!(
        scan.commit_sha.is_none(),
        "commit_sha should be None for empty repo"
    );
    assert!(
        scan.branch.is_none(),
        "branch should be None for empty repo"
    );
}
